...
class RsMoveCommonProcessor(
    ...
    private fun createOutsideReferenceInfo(
        ...
        val pathNewFallback = if (path.containingMod == sourceMod) {
            ...
            // todo добавить тест, раньше здесь была ошибка (вместо `targetMod` был `path`)
            target.qualifiedNameInCrate(targetMod)?.toRsPathInEmptyTmpMod(codeFragmentFactory, targetMod)
        }
    ...
    private fun createInsideReferenceInfo(pathOriginal: RsPath, target: RsQualifiedNamedElement): RsMoveReferenceInfo {
        ...
        val pathNewFallback = run {
            // todo добавить тест, раньше здесь была ошибка (использовался просто **target**.qualifiedNameRelativeTo(path.containingMod))
